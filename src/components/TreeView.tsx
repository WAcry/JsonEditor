...
const parseInputValue = (input: string): { success: boolean, value: unknown } => {
    ...
    if (trimmed !== '' && !isNaN(Number(trimmed)) && String(Number(trimmed)) === trimmed) {
...
const EditableNode: React.FC<{
    ...
    const commitChange = () => {
        ...
        if (parsed.success && JSON.stringify(node.value) !== JSON.stringify(parsed.value)) {
    ...
    if (isEditing) {
        return <input ref={inputRef} type="text" defaultValue={editValue} onChange={e => setEditValue(e.target.value)}
                      onBlur={commitChange} onKeyDown={handleKeyDown}
                      className={`node-value-input node-value-${node.type}`}/>;
    }
    return <span className={`node-value node-value-${node.type}`}
                 onClick={onEditStart}>{JSON.stringify(node.value)}</span>;
};
    ...
    return (
        ...
        {node.hasChildren && (isExpanded ? '▼' : '►')}
          ...
          {node.type === 'array' ? '[' : '{'} {node.childrenCount} {node.childrenCount === 1 ? 'item' : 'items'} {node.type === 'array' ? ']' : '}'}
                ...
                <EditableNode node={node} isEditing={editingId === node.id}
...
function TreeView({data, parseError, onUpdate}: TreeViewProps) {
        ...
        setExpanded(prev => {
            ...
            if (isCurrentlyExpanded) {
                ...
                Object.keys(newExpanded).forEach(key => {
                    if (key.startsWith(id + '.')) delete newExpanded[key];
                });
            } else {
                newExpanded[id] = true;
            }
    ...
    if (parseError) {
        return (
            <div className="tree-container">
                <div className="tree-error">Error: {parseError}</div>
            </div>
        );
    }
    ...
    return (
        <div className="tree-container">
            <FixedSizeList
                height={window.innerHeight - 48} // Adjusted for header height
